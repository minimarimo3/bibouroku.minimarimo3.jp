#import "../template.typ": project, note, note, tip, important, warning, caution
...
文書を単純にHTMLエクスポートするなら普通に`typst compile --features html --format html input.typ`すればよいのですが、この方法でコンパイルするとTypstの内容はすべてbody内に記述されます。
そのため、headなどにいろいろ記述する場合は`html.html`を使用してエクスポートするHTMLの構造を一から記述する必要があります。
このモードはカスタムHTMLモードと言うそうです。
...
しかし、数式をSVG画像に変換してHTMLに埋め込むことができます。 @github2025_html_export_issue_721_typsttyp
